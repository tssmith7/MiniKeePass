...
- (void) initDropboxAPI {
    ...
    if( !isInitialized ) {
        ...
        // Wipeout temp directory to start over from scratch.
...
- (BOOL)isLocalCopyStale:(NSString *)filename {
    ...
    NSString *localRev = (NSString*) localData[DB_REVISION_CODE];
    if( ![dropboxMetadata.rev isEqualToString:localRev] ) {
        // The dropbox revision numbers do not match!  Need to download a fresh copy.
        printf( "Revisions do not match! local('%s'), dropbox('%s')\n", localRev.UTF8String,dropboxMetadata.rev.UTF8String );
        return YES;
    }
...
- (NSArray *)getDropboxFileList {
    // Return a sorted list of dropbox files.
    NSMutableArray *fileList = [NSMutableArray arrayWithArray:[dropboxFileMetadata allKeys]];
    ...
    [[client.filesRoutes uploadUrl:destPath mode:writemode autorename:[NSNumber numberWithBool:NO]
      ...
      setResponseBlock:^(DBFILESFileMetadata *result, DBFILESUploadError *routeError, DBRequestError *networkError) {
          if (result) {
              ...
              localFileMetadata[file] = dict;
              // Change modified date for the local copy.
              [self setModifiedDate:result path:srcPath];
              if( ![localFileMetadata writeToURL:[self getLocalURL:DROPBOX_META_ARCHIVE] atomically:YES] ) {
                  NSLog(@"Error writing local File Metadata Archive\n");
              }
              requestCallback(nil);

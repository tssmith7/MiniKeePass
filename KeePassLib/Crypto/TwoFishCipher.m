...
@implementation TwoFishCipher {
    Twofish_key m_key;
    uint8_t ivec[TWOFISH_BLOCK_SIZE];
}
...
// Make sure the input to Encrypt has a count that is a multiple of 16.
...
-(void) Encrypt:(void*)m iOffset:(size_t)iOffset count:(size_t)cb  {
    ...
    int i, numBlocks;
    uint8_t block[TWOFISH_BLOCK_SIZE], *iv;
    ...
    for(i = numBlocks; i > 0; i--)
    {
        ...
        ((uint32_t*)block)[3] = ((uint32_t*)pInput)[3] ^ ((uint32_t*)iv)[3];
...
-(void) Decrypt:(void *)m iOffset:(size_t)iOffset count:(size_t)cb {
    ...
    uint8_t block[TWOFISH_BLOCK_SIZE];
    uint32_t iv[4];
    uint8_t *pInput = (uint8_t*)m;
    ...
    for(i = numBlocks - 1; i > 0; i--)
    {
        ...
        memcpy(iv, pInput, TWOFISH_BLOCK_SIZE);
 ...
 */
#define Twofish_fatal(pmsgx) { printf(pmsgx); }
